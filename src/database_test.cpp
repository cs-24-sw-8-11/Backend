...
int main(){
    ...
    questions.add_test("add-question", [&](){
        assert(questions.db->questions->size() == questions.default_questions.size());
    });
    ...
    questions.add_test("modify-question", [&](){
        ...
        assert(current_data == questions.db->questions->get(questions.question_id));
        questions.db->questions->modify(questions.question_id, {"tags"}, {"notdefault"});
